...
- 상태: stable
- 최종 수정일: 2026-06-18
- 버전: v0.9.1-beta (chapter 1+2+3+4 done, v0.9.1 chapter 5 done; package: standard-ai-workflow 0.9.1, runtime __version__ = v0.9.1-beta, tag v0.9.1-beta released 2026-06-18)
...
**누적 (v0.8.0 → v0.9.1)**: 16 + 2 release (v0.9.0 chapter 1+2+3+4 + v0.9.1 chapter 5), mypy strict cumulative **19 file clean** (mypy 2.1.0 stricter checking, full strict 도달 v1.0.0 milestone), smoke test **162/162 PASS** (cumulative, v0.8.15+ 유지, 신규 10 test 별도 subset), spec §9 acceptance **9/12** (deprecation 2nd cycle / pipeline `--apply default=False` / consumer signal 2nd wave 후속), breaking change 없음 (SemVer 2-year guarantee, v0.8.0 → 2.0.0 까지). Phase 11 close + Phase 12 kickoff (workflow_kit_roadmap.md §1 갱신, 3/6 완료).
