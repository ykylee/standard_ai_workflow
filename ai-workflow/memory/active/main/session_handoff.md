...
- 현재 주 작업 축: **배포 일관성·멱등성 — 컨셉이 승인됐고 구현이 시작 대기다.** 정본은 [`workflow_deployment_idempotency.md`](../../../../workflow-source/core/workflow_deployment_idempotency.md) (배포=함수, 변수 5축, 3계약+1탐침, 소유권 3분류, 멀티 하네스 공존·설치 스코프 규칙). **구현 순서**: [main-016] `wk doctor` post-apply 탐침(**1순위** — 스코프별 설치 현황·버전·로드 가능성·환경 전제 한 명령) → [main-017] 채널별 재실행 계약 표(플러그인 5채널 실측) → [main-018] 드리프트 감지(마커 스캔) → [main-019] 환경 pre-flight. **release 경계 대기** — [TASK-2026-08-14-main-009] 라벨 영어 전환은 `TASK_FIELD_LABELS` 한 줄만 남았고 case 10 이 안전을 선실증. 45차에 구조 결함(main-005/006/010/011)과 판단 보류(main-004 = 기각)가 전부 닫혀 실행형 잔여는 배포 축뿐이다. **이 호스트 설치 현황(47차 실측)**: Claude Code ✅ user scope v1.2.0 (GitHub marketplace) · Codex ✅ v1.2.0 (로컬 `workflow-source/dist/plugins/codex/1.2.0/install-root`, **gitignore 대상이라 `dist/` 를 지우면 marketplace 가 끊긴다** — 재빌드는 `python3 -m workflow_kit.plugin_distribution --harness codex --output-dir workflow-source/dist`). **미해결 잔여 결함** — [TASK-2026-08-16-main-001] backlog-update 조용한 스킵 · [TASK-2026-08-16-main-003] check_deprecation_3rd_cycle 제외 사망. **관찰** — [TASK-2026-08-13-main-004] mypy flake 33 run 연속 green(진행 중) / cross-host federation / darwin mavis e2e / memory_index 3-tuple 추이.
