...
- 상태: active
- 최종 수정일: 2026-09-23 (87차 세션 **종료** — v1.11.0 환경 배포 정리(Claude 플러그인 1.9.1·시스템 python 1.2.0 이 낡아 있었다) + 중첩 worktree 가 소스 스윕에 섞인 호스트 의존 red 수리)
...
- 현재 기준선: **87차 세션 (2026-09-23, macOS 호스트) — 환경 배포 정리 + 호스트 의존 red 1건 (task 2건 close: main-010·main-011).** ①**main-010** — 이 호스트의 v1.11.0 반영. Claude Code 설치본이 **1.9.1** 에 멈춰 있었다(marketplace 는 `directory` 소스라 이미 1.11.0 을 보는데 설치 기록만 낡음) → `claude plugin update` 후 재시작, 1.9.1 캐시 삭제. **더 숨은 것**: 플러그인 `.mcp.json` 은 `python3 -m workflow_kit…` 만 부르므로 저장소 밖에서는 시스템 python 설치본이 뜨는데, 그것이 옛 worktree 를 가리키는 editable **1.2.0** 이었다. 저장소 안에서는 `PYTHONPATH=workflow-source` 덕에 가려진다 → Release wheel 로 교체, 저장소 밖 cwd 에서 serverInfo.version=1.11.0 실측. Codex·Grok·Antigravity 는 다른 에이전트(Codex 세션)가 먼저 갱신해 두고 **미커밋 `main-009`** 로 남겼는데 원격이 그 ID 를 다른 task 에 먼저 써서, 소유자 확인 후 `main-010` 으로 재번호해 얹었다. `wk doctor` 네 채널 in-sync · 낡은 호스트 0. ②**main-011** — 커밋 전 `--changed` 에서 `check_source_compile_warnings` case 3 red. `python_floor.iter_sources` 가 rglob 에서 `__pycache__`·`.venv*` 만 빼서 **저장소 안 gitignore 된 중첩 worktree**(`.worktrees/feat-auto-20260814-…`)의 옛 소스 521개를 스윕했다 — 그 worktree 가 있는 호스트에서만 red, CI green. 자기 `.git`(파일·디렉터리)을 가진 하위 디렉터리를 가지치기(이름이 아니라 `.git` 존재로, 루트 자신은 판정 안 함). 열거 1094 → 573 = git 추적 전수. case 7 신설, 되주입 3종 각각 다른 지문. 게이트 **586/586**, CI `ca2db5ba` 4셀 + 3 workflow green.
