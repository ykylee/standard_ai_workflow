...
> **`TASK-2026-09-21-main-009` — 하한 호환 축의 범위가 `workflow-source/` 로 좁다 (planned).**
> 83차가 컴파일 경고 스윕의 범위를 **git 추적 `*.py` 전수(565)** 로 파생시키면서
> 드러났다. `python_floor` 의 `probe`/`main` 과 `check_python_floor_syntax` 는
> `source_root = workflow-source` 로 잡는데, 그 밖에 git 추적 `.py` 가 **8개** 있다 —
> `main.py` · `ai-workflow/mcp_servers/*/scripts/run_*.py` 6종 ·
> `scripts/audit_mkdocs_links.py`. 그중 MCP 서버 스크립트는 **실제로 배포·서빙된다.**
>
> **결함은 0 이다** (실물 3.10 컴파일 실패 0 · 3.13 경고 0, 2026-09-21 실측). 급한
> 것이 아니라 *범위가 선언 의도보다 조용히 좁은* 것이고, 이 저장소가 포함 목록으로
> 이미 세 번 겪은 모양이다. 덤으로 `_partial_sweep` 이 열거를 한 벌 더 손으로 갖고
> 있고 `.venv*` 제외가 빠져 있다 — `iter_sources` 로 접을 수 있다.
>
> 남의 축을 조용히 넓히지 않으려고 83차가 **일부러 task 로 남겼다.**
