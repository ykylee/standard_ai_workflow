...
> **`TASK-2026-09-22-main-002` — 발행 절차가 거짓 문서 스탬프를 만든다 (planned).**
> `doc-headers-update --apply` 가 **내용이 한 줄도 안 바뀐 문서 100개**의
> `최종 수정일` 을 오늘로 올린다. 되돌리고 커밋해도 **`release --apply` 의
> post-step 이 똑같이 다시 넣는다** — 커밋 전 되돌림으로는 못 막는다.
> 두 번 다 되돌린 뒤 관련 검사 6종이 전부 green 이라 **어떤 검사도 요구하지 않는**
> bump 다. v1.9.2 발행은 144 파일이었으니 과거에 이미 실려 나간 적이 있고,
> 그래서 일부 문서의 현재 스탬프가 이미 거짓일 수 있다.
...
> 드러났고, 그중 **열린 채로 남은 것은 `main-002` 하나**다. **발행 절차를 밟는 것 자체가 탐침이었다** — 평소 경로로는 안
> 밟히는 자리들이다.
