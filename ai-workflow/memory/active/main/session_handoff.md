...
1. **codex 호스트 3개 재시작** (즉시, 작업 아님 — 사용자 몫). 54차가 codex 설치본을
   ...
   노출되지 않는다. claude-code 쪽은 이미 해소됐다(이 세션 프로세스가 설치 이후).
2. **혼합 표기 코퍼스의 후속** — 저장소 task 파일은 이제 한국어/영어가 섞였다. 읽는 쪽은
...
3. **포크 분류의 소비자 쪽 후속** (54차 후속, 선택) — `CLAUDE.md` 는 이제
   `v1.0.0-beta` 에서 포크됐다고 선언한다. 그 버전 생성물과 diff 하면 **놓친 kit
   변경**(영어 진입 규칙 · `<branch>` 설명 줄 등)이 보인다. 병합할지, 포크를 그대로
   둘지는 결정 사항이다 — 도구는 이제 어느 쪽도 강요하지 않는다.
4. **OKF v0.2 이행 ADR 후속** (main-006 후속) — `status` 어휘가 실질 위험.
5. **[TASK-2026-08-13-main-004] mypy flake 관찰** — 완료 기준은 '다음 재발이 트레이스백을
   로그에 남길 것'. 아직 재발 대기.
