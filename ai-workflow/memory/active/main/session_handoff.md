...
- 현재 기준선: **33차 세션 종료 — Codex·Claude Code native plugin release asset 분리 (TASK-2026-08-13-main-008).** 브랜치 `feat/plugin-harness-distribution`에서 공유 `plugin/` payload에 Codex `.codex-plugin/plugin.json`을 추가하고, `workflow_kit.plugin_distribution.PLUGIN_HARNESS_SPECS` 레지스트리로 Codex/Claude Code ZIP을 별도 생성한다. `wk release-pipeline dist --apply`는 wheel/sdist와 함께 `standard-ai-workflow-{codex|claude-code}-plugin-<version>.zip`을 만들고, `release`는 두 ZIP이 없으면 fail-closed하며 있으면 GitHub Release asset에 첨부한다. Codex ZIP은 extract 후 `codex plugin marketplace add` → `codex plugin add standard-ai-workflow@standard-ai-workflow`이 가능한 marketplace 구조다. 검증: payload 17/17, plugin distribution smoke, Codex manifest validator, actual dist apply, release dist regression 8/8, ruff target 모두 PASS. 전역 mypy strict은 기존 의존 모듈 48 오류로 실패했으나 새 `plugin_distribution.py` 자체는 strict/follow-imports=skip PASS. **변경은 커밋·푸시되어 PR #23에서 검토 중**이며, 다음 세션에서는 리뷰 후속 조치와 CI 결과를 확인한다.
