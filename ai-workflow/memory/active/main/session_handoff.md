...
1. **codex 호스트 3개 재시작** (즉시, 작업 아님 — 사용자 몫). 54차가 codex 설치본을
   1.2.0 → **1.3.0 으로 올렸다** (`content_drift` 0). 그런데 pid 6191(07-31) ·
   97626(08-07) · 30424(08-16) 가 설치(08-20 14:58)보다 먼저 시작해 있어,
   `runtime_load` 가 낡음으로 잡는다 — 재시작 전까지 그 프로세스에는 새 스킬이
   노출되지 않는다. claude-code 쪽은 이미 해소됐다.
2. **OKF v0.2 이행 ADR 후속** (main-006 후속) — `status` 어휘가 실질 위험.
   **남은 것 중 유일하게 큰 축이다.**
3. **혼합 표기 코퍼스의 후속** — 저장소 task 파일은 한국어/영어가 섞였다. 읽는 쪽은
   `TASK_FIELD_ALIASES` 가 덮지만, 새 리더가 ALIASES 를 안 거치고 리터럴로 비교하면 같은
   계열의 결함이 다시 난다. 그물은 `check_task_label_registry` case 12·13 과
   `check_task_ssot_source` case 11 이다. 기존 파일 일괄 마이그레이션은 **하지 않기로**
   한 상태(별칭이 창구) — 뒤집으려면 결정이 필요하다.
4. **[TASK-2026-08-13-main-004] mypy flake 관찰** — 완료 기준은 '다음 재발이 트레이스백을
   로그에 남길 것'. 아직 재발 대기.
