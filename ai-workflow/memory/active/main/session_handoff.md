...
- 상태: active
- 최종 수정일: 2026-09-21 (83차 세션 **종료** — 경고 게이트의 두 구멍: 부모 프로세스 경고 미수집 + 판정이 __pycache__ 에 달려 재실행만으로 green)
...
- 그 이전 기준선: **82차 세션 (2026-09-21, plex 호스트/linux) — 검사를 *무엇으로 돌리는가* 를 선언으로 옮겼다. 저장소가 같은 모양을 고친 세 번째 축이다 (task 1건 close: main-006).** **① 착수 전 실측이 설계를 갈랐다.** 3안(ⓐ전량 2축 / ⓑ선언 부분집합 / ⓒ하이브리드) 중 **ⓑ를 기각**했다 — 전량을 두 해석기로 대조하니 `287/287 ×2 green` 인데 **출력이 갈린 검사가 8건**이었고, 민감한 쪽은 *저장소 소스를 파싱하는 검사* 라 미리 목록화할 수 없다. 저장소는 브랜치 축에서 같은 모양의 포함 목록을 이미 두 번 기각했다(§2.53 · smoke.yml 주석). **② 더 고약한 것이 있었다 — 3.13 커버리지가 선언이 아니라 *우연* 이었다.** CI 는 검사를 3.11 하나로만 돌았고 3.13 은 개발자 `.venv` 에 깔린 것이다. `.venv` 를 3.11 로 다시 만들면 그 커버리지는 **조용히 사라지고 아무 검사도 실패하지 않는다** ('우연한 측정은 선언이 아니다' 재발). **③ 축은 가정이 아니라 지금 열려 있었다**: 3.12+ 에서만 나는 `SyntaxWarning: invalid escape sequence` 를 5개 검사가 3.13 에서만 보고 **3.11(CI)은 완전히 침묵**했다. 출처는 81차가 리스크로만 적어 둔 `check_v0_7_26_sync_release_hash.py` 7·93행 — r-string 으로 수리했다. **방향이 81차와 반대다**(81차: 로컬 green / CI red). 즉 두 방향 모두 실증됐다. **④ 정본의 방향을 뒤집었다.** task 등록 때는 '기대값을 smoke.yml 에서 파생' 이었는데, 그러면 *사본이 정본이 된다* — `branch_matrix`/`sdk_matrix` 와 같이 **registry → yml** 로 세웠다 (`workflow_kit/common/interpreter_matrix.py` 의 `GATE_INTERPRETERS`, prepare job 이 주입). `dev-local` 선언은 `.python-version` 과 묶어 저장소 사실에서 떠다니지 않게 했다. **⑤ 비용은 CI 에 두고 로컬 게이트는 안 늘렸다**: CI smoke 는 브랜치 2 × 해석기 2 = **4셀** (병렬이라 벽시계 불변), 로컬은 재현 수단 `interpreter_matrix --run-local` (venv 는 `.venv-interpreter-matrix/<버전>` 캐시). CLAUDE.md 가 게이트 벽시계를 명시적으로 관리하므로 로컬 전량 반복은 기각했다. **⑥ 범위는 손 목록이 아니라 파생이다**: `.github/workflows/` 를 훑어 `run_all_checks.py` 를 부르는 workflow 를 뽑고, 그중 registry 도 안 읽고 면제 선언도 없는 것을 red 로 잡는다 (`mcp-sdk-matrix.yml` 은 이유와 함께 면제 — 그 부분집합은 smoke 4셀이 덮는다). **⑦ 되주입이 내 결함 3건을 잡았다**: `case 4` 가 파일 어디든 `matrix.python` 문자열이 있으면 통과해 **정작 setup-python 배선을 끊어도 green** 이었다(배선 자체를 보도록 교체) · `WATCHES` 가 `pyproject.toml` 누락(meta-watch 실측) · 부모 stdout 버퍼링으로 셀 헤더가 뒤집혀 출력. **⑧ 되주입 harness 자신이 한 번 거짓말했다** — 주입 두 건의 소스 **길이가 같아** `.pyc` 가 유효 판정돼 재사용됐고 case 2 가 case 1 의 증상을 냈다. `PYTHONDONTWRITEBYTECODE` + pyc 삭제로 고쳤다. *주입이 실제로 적용됐는지 확인하라* 는 규율이 harness 에도 적용된다. **⑨ 검증**: 되주입 **10종**(case 9종 + 축 자체 1종) 전부 red → 복원 byte-identical green. 축 되주입은 호스트 의존 판정을 심어 **3.13 PASS / 3.11 FAIL** 로 CI 전용 red 를 로컬에서 재현한 것이다. 커밋 전 `--changed` 271/271 · `check_self_application` 8/8 · push 게이트는 새 4셀을 로컬에서 그대로 밟아 **288/288 ×4 green** (py3.13 378s · py3.11 373s). **⑫ main-007 — 경고를 게이트 신호로 (같은 세션에 닫음).** 수집 지점은 새로 만들 것이 없었다: `run_one` 이 이미 `stdout+stderr` 를 전량 받아 두고 요약 뒤 **버리고** 있었다. **전수 census(288검사 × 2해석기)가 판정 경계를 정했다** — 저장소 안 6/6, 서드파티 0/2이고 **갈린 2건은 전부 서드파티**였으며 원인도 해석기가 아니라 두 venv 의 의존성 해석 차이였다 (`pydantic_settings`). 그래서 등록 때 적어 둔 2안을 둘 다 기각했다: (B) 해석기 간 출력 차이 = 오늘 당장 위양성 2건이고 실제로 다른 버전이라 정규화로도 안 없어진다 · (A) 전역 red = 8건 중 6건이 의도된 deprecation 호출이라 즉시 만성 red. **채택은 (A) 를 출처(위치)로 좁힌 것**이고, 그러자 등록 때의 리스크 '비교 지점이 없다' 가 **사라졌다** — 각 셀이 독립 판정해도 3.12+ 에서만 나는 경고는 그 셀이 red 가 된다. `<unknown>` 은 게이트 대상으로 뒀다(82차 결함의 위치가 정확히 그것이다 — 서드파티로 밀면 이 축을 만든 결함이 빠져나간다). 저장소 안 6건은 전부 81차의 deprecation 경로를 의도적으로 부른 것이라 `call_deprecated` 로 감쌌다 — **삼키되 경고가 났는지 단언**하므로 면제 registry 없이 베이스라인이 0 이고 deprecation 이 사라지면 그쪽이 red 가 된다. 되주입 7종 전부 red→복원 green이며 **그 되주입이 내 검사의 약한 case 2개를 잡았다**: case 1 은 표본에 `Warning:` 토큰 든 산문이 없어 *위치 요구를 지워도 통과*했고, case 3/5 는 표본이 전부 `.venv` 라 `site-packages` 분기를 한 번도 안 밟았다. 게이트도 내 결함 2건(타입 주석 누락 · smoke 수 미반영)을 잡았다. 검사 287→289, 게이트 **289/289 ×4 green**, push 후 CI 5종 전부 success (smoke 4셀 포함, `5205ba80`). **⑬ 세션 최종**: 닫은 task 2건(main-006 · -007), 검사 287→289(신설 2 · 보강 3), 되주입 **17종**(main-006 10 · main-007 7) 전부 red→복원 byte-identical green, 커밋 4개(`e5f9b554` `a91549ba` `4a6079f0` `5205ba80`), CI 3회 전부 success. `wk suggest-memory-entries` 는 §4 10건 중 덮인 것 **0/10** 으로 후보 10건을 냈다 — 승격은 소유자 결정 축이라 **write 하지 않았다**(advisory). **⑩ push 후 CI 확인까지가 검증이다** (매트릭스 구조를 바꿨으므로): 워크플로 6종 전부 success 이고 smoke 는 선언대로 **4셀**이 떴다 — `native/py3.11` · `native/py3.13` · `slash/py3.11` · `slash/py3.13` (run 35562623777). prepare job 을 `setup-python` 없이 러너 기본 python3 로 돌리는 것도 실전에서 섰다. **⑪ 그 잔여는 `main-007` 로 같은 세션에 닫혔다** — 저장소 코드가 낸 경고는 이제 게이트 red 다 (§ 아래 ⑫). `SyntaxWarning` 류를 red 로 올릴지는 별도 판단 — 올리면 3.12+ 셀만 red 가 되는 부류가 생긴다.
- 그 이전 기준선: **81차 세션 (2026-09-21, plex 호스트/linux) — 지표 하나가 무엇을 재고 있었는지 뒤집고, 그 수법을 선언 축으로 일반화했다 (task 3건 close: main-001 · -002 · -003).** **① `graph_insights` 는 어휘를 재고 있었다.** coverage 4개 goal 전부 겹침이 **정확히 0** (0/13 · 0/14 · 0/12 · 0/15)이었고, 조사 제거·CJK bigram 두 대안 토크나이저로 다시 재도 최대 0.07 — 그 유일한 겹침은 기능어 `처럼` 이었다. 원인은 토크나이저도 임계도 아니라 **입력 쌍**이다 (Goals 는 전략 산문, 완료 항목은 결함수리 제목). 분류 축도 같은 결함을 공유해 미분류를 면한 2건의 근거가 전부 동음이의(`runtime` / `흡수`)였다 — 즉 6/100 은 전부 잡음이었다. 안 잡힌 이유는 **fixture 가 항등함수를 쟀고**(goal 문자열을 done 항목에 통째로 복사) 그걸 막는 가드가 문자열 **완전일치만** 봤기 때문이다. 소유자 결정 = 선언 기반 교체: `task.wbs → milestone.wbs_goals/goals → PURPOSE §1`. **leaf 단위 선언**을 넣은 것은 실측이 강제했다 — 최근 완료 10건이 **전부 M-007** 이라 마일스톤 단위면 0 대신 *다른 상수*가 됐을 뿐이다. 못 잰 것은 `undeclared`/`unmeasured` 로 내보내고 `provenance` 가 무엇을 채우면 닿는지 적는다. 저장소 실측 **0.0/poor(6) → 75.0/excellent(82)**, 미분류 10→0, G4 만 uncovered. **② `check_wiki_trend` 의 만성 red 는 두 층이었다.** 진짜 신호 = L2 stub 4종 전부 32일 경과(상한 30) → `--emit-l2 --apply` 로 lifecycle 0.00→5.00. 판정 결함 = `rc == 0` 을 **살아있는 저장소 점수**에 걸어 둬 달력만으로 red 가 됐다 → '종료 코드가 자기가 출력한 alert 수와 일치하는가' 로 교체(결정적). 신호를 잃지 않게 **구조적 방치만** 게이트하고 경과일은 보고만 한다. **③ OpenSpec 조사 → 도구 채택 기각, concept 만 흡수** (소유자 결정). 근거 실측: 검사의 `§` 인용 **671건 / 127파일** 중 기계가 문서를 특정할 수 있는 것은 **10건** — 끊긴 링크가 아니라 *링크가 아닌 것* 이 본체라 산문 변환은 성립 안 한다. 그래서 `ENFORCES` 선언 축을 신설했다(`WATCHES` = 무엇이 바뀌면 나를 돌려라 / `ENFORCES` = 내가 무엇을 지킨다). 정본 = `test_impact_tiering_spec.md` §7. **④ 검증**: 되주입 **10종**(main-001 4 · main-002 2 · main-003 4) 전부 red → 복원 byte-identical green. 되주입이 **죽은 판정 1건**을 잡아냈다 — `check_requirement_declarations` case 3 은 헤딩 정규식이 kebab-case 만 인식해 형식 위반이 red 가 아니라 *침묵*했고, 그러면 case 1 이 스펙 대신 검사를 탓한다. `check_graph_insights_health_monotonic` 4→6 case · `check_roadmap_integrity` 6→8 · `check_wiki_trend` 13→15 · `check_requirement_declarations` 신설 4 case. push 게이트 전량 2축 **284/284 ×2 green**. **⑤ 잔여**: `ENFORCES` 미선언 282/285 는 **의도된 관찰 지표**다 — 전수 강제하면 선언의 품질이 아니라 개수만 는다. **⑥ 같은 모양을 한 번 더 만났다 — 범위가 포함 목록이었다 (main-004).** smoke 수 284→285 로 게이트가 문 자리를 따라가 보니, `check_code_index` / `check_installation_usage` 는 이미 디스크에서 세고 있었고(파생 O) **각자 자기 문서 하나씩만** 봤다. 목록 밖은 조용히 갈라져 있었다 — README 162(2곳) · `workflow_kit_roadmap` 52(+미러) · `.omo` 52, 실제 285. 수정은 두 갈래: **수가 짐을 지지 않는 자리에서는 수를 뺐고**(문장의 요지는 커버리지 범위지 개수가 아니다) 수는 게이트가 덮는 2곳에만 남겼다. 신규 `check_smoke_count_claims` 3 case 가 git 추적 md **1321건 전수**를 훑고, 동결은 이름이 아니라 **문맥 두 신호**로 파생한다 (①문서 메타데이터 헤더 부재 ②기록 계층 — 경로는 `memory_dir_for_workspace` 에서 얻는다). **설계가 한 번 틀렸다**: 헤더 신호 하나로 8건이 갈린다고 봤는데 전수에서 `memory/release/v0.5.10/backlog/` 가 오분류됐다(생성기가 헤더를 달아 준다) — **손 조사가 놓친 것을 전수가 잡았다.** 되주입 4종 중 하나는 *동결 문서를 999로 바꿔도 green* 으로 위양성 없음을 실증한 것이다. **⑦ CI 에서만 터질 문법을 쓸 뻔했다 → main-005 로 등록(planned).** 중첩 f-string(PEP 701, 3.12+)인데 로컬 .venv 는 3.13.7 이고 **선언 하한은 `requires-python = ">=3.10"`** 이다 (CI 워크플로는 3.10/3.11/3.12 혼용 — 기대값의 출처는 CI 매트릭스가 아니라 선언이다). 실측: 소스 551개를 3.10/3.11/3.12 로 각각 파싱해 실패 0 — **예방 축**이다. 덤으로 `check_v0_7_26_sync_release_hash.py` 7·93행의 invalid escape sequence 를 발견해 리스크에 적었다. **이 세션 누적**: 닫은 task 4건(main-001·-002·-003·-004) + 연 task 1건(main-005), 검사 284→286(신설 2 · 보강 3), 되주입 **18종** 전부 red→복원 green, 게이트 286/286 ×2, 커밋 4개(`c08dfebd` `fa4b1cb4` `d882b42a` `d4550ec3`). **⑧ main-005 까지 닫았고, 착수 실측이 또 전제를 뒤집었다.** 등록 때 설계는 `ast.parse(feature_version=floor)` 였는데 그 수단은 **이 축을 만들게 한 결함을 못 잡는다** — PEP 701(중첩 f-string, 3.12 도입)은 토크나이저 층 변경이라 파서의 버전 게이트에 안 걸린다 (실측 7종 중 거부한 것은 type alias / type parameter / `except*` 3종뿐). 따라서 main-004 에서 '3.11 호환 확인했다' 고 한 것도 근거가 아니었다. **실물 해석기로 갈아탔다**: `workflow_kit/common/python_floor.py` 가 `requires-python` **선언**에서 하한을 파생하고(CI 매트릭스가 아니다 — CI 는 3.10/3.11/3.12 혼용) `uv` 로 얻은 3.10.20 으로 553개를 **메모리 `compile()`** 한다(`py_compile` 은 `__pycache__` 로 저장소를 오염시킨다 — 신규 0 확인). 판정은 `sdk_matrix` 규율대로 **긍정 증거**. **CI 제약은 감추지 않았다**: CI 에 `uv` 도 3.10 도 없어 부분 측정으로 내려가되 `[unmeasured]` 두 줄로 못 잰 부류(PEP 701)와 전수로 올리는 법을 출력한다. 되주입 5종 전부 red→복원 green(그중 둘은 *부분 측정의 한계가 사실인지* 를 잰 것이다). **게이트가 내 결함 2건을 잡았다**: `main()` 이 `Path(__file__).parents[3]` 로 체크아웃 레이아웃을 역산했고(이 저장소가 **여섯 번** 고친 결함족, 정본 `resolve_workspace_root()` 가 있는데 참조 안 함) meta-watch 좁은 선언 38건. **세션 최종**: 닫은 task 5건(main-001~-005), 검사 284→287(신설 3 · 보강 3), 되주입 **23종** 전부 red→복원 green, 게이트 287/287 ×2, 커밋 6개(`c08dfebd` `fa4b1cb4` `d882b42a` `d4550ec3` `07ad98f9` `9915e4ad`).
- 그 이전 기준선은 [`baselines.md`](./baselines.md) 에 있다 (이관 97건, 최신이 위).
