...
- 그 이전 기준선은 [`baselines.md`](./baselines.md) 에 있다 (이관 85건, 최신이 위).
...
> 게이트가 잡는다.** 단 이 수리는 **아직 미발행** — v1.8.0 소비자에게는
> `workflow_kit.cli` 가 여전히 없다.
