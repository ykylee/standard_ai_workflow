...
>
> **남은 것은 소급이다.** v1.9.2 발행이 144 파일이었으므로 과거에 이미 올라간
> 스탬프가 있고, 그 문서들의 `최종 수정일` 은 실제 변경일보다 앞서 있다. 규칙이
> `>=` 라 검사는 안 잡는다(앞선 것은 거짓이 아니라는 규약) — 고치려면 별도 판단이
> 필요하고 84차는 하지 않았다.
