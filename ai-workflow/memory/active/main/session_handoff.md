...
- TASK-2026-08-05-main-004 세 번째 사본은 이미 갈라져 있었다 + 자기 자신과 비교하는 검사
- TASK-2026-08-05-main-005 승격 조건을 못 적은 게 아니라 적을 수 있는 조건이 없었다
...
- TASK-2026-08-06-main-003 학습회 마스터 HTML 발표 덱 38장 슬라이드 헤더 min-height(104px) 및 본체 margin-top(24px) 수직 고정 정렬 표준화 완료
...
- `core/global_workflow_standard.md` **§10 "다중 작업과 협업"** 신설 (10.1 격리 /
  10.2 확인 후 선점 / 10.3 공유 파일 / 10.4 사용자 확인). 원칙만 담고 근거는
  `multi_workspace_orchestration.md` 로 링크 (표준은 얇게).
- **§1 공통 원칙 bullet 2건 추가 (6→8)** — 이 자리가 `standard_rules` 로 추출돼
  **12개 하네스 진입점에 자동 주입**된다. 연쇄 재생성 4곳 완료: 스냅샷 / 배포본 사본 /
  `CLAUDE.md` / `.claude/.../SKILL.md` (진입점은 정본 렌더러로 생성, 각 +2줄).
- **전파 실측**: 빈 저장소에 `bootstrap --harness codex --harness gemini-cli` →
  `AGENTS.md` 2/2, `GEMINI.md` 2/2 신규 규칙 포함. 신규 소비자는 자동으로 받는다.
- **⚠️ 규칙만 반영됐고 도구는 미착수** — registry / seed / stale 보고가 없어 당분간
  절차를 손으로 지켜야 한다.
- **사고 1건(복구됨)**: `CLAUDE.md` 섹션 치환 시 `str.index` 가 본문 인용구(L14)에 먼저
  걸려 40줄이 날아갔다. `git checkout` 복원 후 `re.M` 줄 앵커로 재작업.
...
- **이번 세션의 교훈(§2.66, 제거는 검사가 완주시킨다)**: `transport_ready` 를 wire 4층에서
  빼면서 옛 필드를 고정하던 검사 8종을 새 축으로 옮겼는데, 그중 **3종은 옮기다 놓쳤고**
  전량 smoke 가 아니라 그 검사들이 직접 `KeyError: 'bridge_phase'` / `'transport_ready'`
  로 드러냈다. 필드를 지우는 작업에서 **그 필드를 붙잡고 있던 검사가 곧 체크리스트다** —
  하나씩 red 가 되면서 남은 자리를 알려 준다.
- **이번 세션의 교훈(§2.66, 문서에 붙인 생성 산출물은 이미 갈라져 있었다)**:
  `mcp_servers/read_only_bundle.md` 에 붙어 있던 JSON 사본이 `tool_count: 12`(실제 13),
  `tool_version: v0.4.1-beta`(실제 v1.0.0-beta), `apply_robust_patch` 누락 상태였다.
  **아무도 그 블록이 틀렸다는 것을 몰랐다** — 문서 안의 사본은 검사가 안 보기 때문이다.
  고치지 말고 **없애고 정본 포인터로** 바꿀 것.
- **이번 세션의 교훈(§2.65, 금지 범위는 이름이 아니라 문맥으로 정한다)**: 버전 고정 SDK
  필드 이름을 저장소 전역에서 금지하려 했더니 **30건 중 28건이 위양성** 이었다 —
  rename 된 53쌍에 `task_id`, `created_at`, `max_tokens` 처럼 **우리 객체에서도 흔한
  이름**이 섞여 있다. 범위를 AST 로 판정한 "실제 mcp 를 import 하는 파일" 7개로 좁히니
  히트 2건 위양성 0. **무엇을 금지할지보다 어디서 금지할지가 판정의 품질을 정한다.**
- **이번 세션의 교훈(§2.65, 손으로 고른 목록은 처음부터 불완전했다)**: `isError` 하나인 줄
  알았는데 5개 클래스만 훑은 결과였고, `mcp.types` 전체를 introspect 하니 **53쌍**이었다.
  목록을 설치된 SDK 와 대조하는 case 를 함께 두지 않았으면 그 불완전함을 몰랐을 것이다 —
  **손 목록을 만들 거면 그것이 사실과 맞는지 묻는 검사를 같이 둘 것.**
- **이번 세션의 교훈(§2.64, 앎이 있어도 재현 수단이 없으면 다시 밟는다)**: 새로 쓴 검사가
  `CallToolResult.isError`(1.x 이름) 를 써서 matrix 2.0.0 셀만 red 였다. 이 저장소는 그
  함정을 **이미 알고 있었다** — `read_only_mcp_sdk` 주석과 `check_mcp_lowlevel_sdk_compat`
  계약 6번이 정확히 그것을 적어 뒀다. 그런데 그 검사는 *production 패턴* 만 보고,
  개발 venv 는 하한(1.27.0) 하나뿐이라 **로컬에서 재현할 방법이 없었다.** 앎을 문서와
  검사에 적어 두는 것만으로는 부족하고, **밟을 수 있어야 한다** — `sdk_matrix --run-local`.
- **이번 세션의 교훈(§2.64, 조사 결과가 예상과 달랐다)**: "우리 mcp 를 2.0.0 에 맞게 다
  고쳐야겠다" 로 시작했는데 고칠 production 이 **0건** 이었다. 생성 시 camelCase kwarg 는
  2.0.0 이 alias 로 받고(이미 정합), `protocolVersion`/`listChanged` 문자열은 **프로토콜
  wire 필드명** 이라 SDK 필드명과 무관하다. 깨진 것은 새 테스트 하나였다 — **"다 고쳐야
  한다" 는 직감을 전수 조사로 먼저 검증할 것.**
- **이번 세션의 교훈(§2.64, 자기 것은 TODO 였다)**: `CLAUDE.md` 의 "프로젝트 실행 기본값"
  5항목이 **전부 placeholder** 였다. 실행 기본값 규약을 배포하는 저장소가 자기 것은 한 번도
  채우지 않았다 — 새 세션은 저 자리를 읽으므로 TODO 는 "명령이 없다" 가 아니라 "찾지 못했다"
  로 이어진다. case 2d 로 되돌아가는 것을 막았다.
- **이번 세션의 교훈(§2.63, 적을 수 있는 조건이 없었다)**: "`transport_ready` 승격 기준을
  정하자" 로 시작했는데, 답은 **그 질문이 성립하지 않는다** 였다. 한 boolean 이 서로 다른
  질문 셋에 답하고 있었다 — 런타임 능력 / 구현 단계 / 정책. 특히 registry 의 상수 `False` 는
  참·거짓을 가릴 명제가 아니었다: **registry 는 어느 transport 가 자기를 서빙할지 모른다.**
  조건을 못 적은 게 아니라 적을 수 있는 조건이 없었던 것이고, 그래서 영원히 false 였다.
  §2.39(`recorded` 가 진행 상태가 아니라 *출처* 였던 건)와 같은 모양 — **어휘가 모자라
  보이거나 임계값이 안 정해지면 먼저 축이 섞였는지 의심할 것.**
- **이번 세션의 교훈(§2.63, 기준은 문서가 아니라 실행이다)**: 축을 쪼개고 나니 기준이
  필요한 것은 정책 축 하나였고, 그것을 문서에 적는 대신 **실행되는 검사**로 뒀다.
  `active_ok` 를 선언하면 검사가 실제로 서버를 띄워 증명을 요구한다. 되주입 2방향이
  다르게 나온다는 것도 확인했다 — 과대 선언은 FAIL, 과소 선언은 `[info] 승격을 검토할 것`.
  판정이 한쪽으로만 작동하면 보수적인 거짓말을 못 잡는다.
- **이번 세션의 교훈(§2.62, 자기 자신과 비교하는 검사)**: 방언 표 `MCP_CONFIG_ROOT_KEY` 를
  만들고 "표가 렌더러 산출물과 같은가" 검사를 짰다. **렌더러도 같은 표를 읽으므로 표를
  바꾸면 양쪽이 함께 바뀐다** — `opencode` 를 옛 값으로 되돌려도 6/6 PASS 였다. 되주입을
  안 했으면 그대로 커밋했을 것이다. 정본으로 접고 나면 *내부* 대조는 전부 동어반복이 되고,
  **판정에는 반드시 외부 증인이 필요하다.** 여기서는
  `examples/mcp_config_examples/opencode-mcp.json` 이 그것이었고 — 처음부터 `mcp` 를
  쓰고 있었다. 증인은 내내 있었고 아무도 대조하지 않았을 뿐이다.
- **이번 세션의 교훈(§2.62, 사본은 갈라진다 — 이미)**: "세 번째 사본인가" 를 확인하러
  갔더니 사본이 셋이었고(alias / codex command·args / opencode command·args + entrypoint 2),
  **이미 갈라져 있었다**: draft 예시가 OpenCode 최상위 키로 `mcp_servers` 를 가르치는데
  실제 렌더러는 `mcp` 를 낸다. 예시대로 붙여넣으면 서버를 못 본다. "사본은 반드시
  갈라진다" 가 이번엔 예측이 아니라 **이미 일어난 사실** 이었다.
- **이번 세션의 교훈(§2.61, 사본을 접으면 사본이 가리던 것이 나온다)**: Grok 의 하드코딩된
  MCP 블록을 정본 helper 로 접는 건 작은 일이었다. 값은 **접는 과정**에서 나왔다 —
  before/after 를 대조하려고 산출물을 `tomllib` 으로 파싱해 보다가,
  `tool_descriptions` 의 키가 `workflow_kit.read_only = "…"` 로 따옴표 없이 나가
  TOML dotted key 규칙에 따라 `{{workflow_kit = {{read_only = …}}}}` 로 **중첩**되는 걸
  발견했다. 코드는 키 하나를 쓴다고 말하는데 파일은 다른 구조를 뜻했다. Codex 에
  처음부터 있었고 Grok 이 베낀 것이라 양쪽 동일 — **접어 뒀기에 한 줄로 둘 다
  고쳐졌다.** 기존 검사는 *커밋된 예시 파일*만 파싱했고 생성기 출력은 아무도 파싱해
  본 적이 없었다(§2.59 의 축이 MCP 층에서 반복).
- **이번 세션의 교훈(§2.61, 되주입은 한 줄만 되돌린다)**: 첫 되주입 시도에서 `git stash` 로
  `mcp.py` 를 통째로 치웠더니 helper 자체가 사라져 import 가 죽었다. 검사는 red 였지만
  **그건 결함 상태가 아니라 다른 상태**다 — 파일 단위 stash 로 되주입하면 "고친 것"이
  아니라 "없앤 것"을 재게 된다. 되돌릴 것은 *그 한 줄* 이다.
