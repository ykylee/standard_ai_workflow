...
1. **릴리스 경계 — 소유자 결정 대기.** v1.2.0-beta 이후 **101 커밋**(feat 17 · fix 24 · breaking 1). 도구가 근거를 갖춰 **2.0.0** 을 제안하고 `requires_decision` 을 세웠다.
   쟁점: `feat(okf)!`(OKF v0.2)를 major 로 볼 것인가. 반론 근거 — 공개 Python API 시그니처는 안 깨졌고,
   은퇴한 진입점 2종은 **남아서 rc=0** 이며, 우리 번들은 legacy 형태를 유지해 v0.1 소비자도 그대로 읽는다.
   SPEC 자신도 v0.2 를 'a minor version bump' 로 규정한다. 이 결정이 **[main-009] 라벨 영어 전환**을 푼다.
2. **[main-009] 라벨 영어 전환** — release 경계 대기. `TASK_FIELD_LABELS` 한 줄만
