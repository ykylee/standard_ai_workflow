...
> **하한 호환 축의 범위는 `main-009` 로 닫혔다 (83차, 같은 세션).** 컴파일 대상
> 루트가 **저장소 루트** 이고, `check_python_floor_syntax` case 4 가 git 추적 `*.py`
> 전수(566)와 대조해 **좁아지면 red** 다. 컴파일 경고 스윕도 `resolve_scope` 로
> 같은 루트를 쓴다 — 두 축이 각자 루트를 정하면 한쪽만 좁아져도 아무도 모른다.
>
> **선언 출처를 같이 보지 않으면 넓히는 것이 위험하다.** 저장소 루트의
> `pyproject.toml` 은 배포되지 않는 placeholder scaffold 이고 `>=3.13` 을 선언한다.
> 선언까지 루트로 옮기면 하한이 3.10 → 3.13 이 되어 **이 축이 아무것도 재지 않는다**
> (개발 해석기가 이미 3.13 이다). 단일 출처는 `python_floor.packaged_declaration`
> 이고 case 5 가 그것을 동결한다.
