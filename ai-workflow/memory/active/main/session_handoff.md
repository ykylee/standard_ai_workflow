...
- 그 이전 기준선은 [`baselines.md`](./baselines.md) 에 있다 (이관 59건, 최신이 위).
...
배포 축은 48차에, wiki L2 축은 49~50차에 닫혔고, **53차가 배포 축의 잔여 두 칸
(`runtime_load` · `installPath`)과 task SSOT 4단계를 닫았다.** 실행형 잔여가 없다 —
**다음 축은 소유자가 고른다.**

후보 (준비 상태 순):

1. **이 호스트의 실사용 복구 — CLI 재시작** (즉시, 작업 아님). 플러그인은 이 세션에서
   1.2.0 → **1.3.0 으로 올렸다.** `runtime_load` 가 지금 pid 77951 을 낡음으로 잡고 있다
   — 재시작 전까지 새 스킬이 노출되지 않는다. codex 사본은 여전히 1.2.0 drift 상태다.
...
3. **OKF v0.2 이행 ADR 후속** (main-006 후속) — `status` 어휘가 실질 위험.
