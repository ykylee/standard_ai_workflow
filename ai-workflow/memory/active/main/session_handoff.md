...
- `TASK-2026-08-25-main-017` — MCP emit command 가 항상 python3.
  **62차에서 코드 수리 완료 + v1.6.0 으로 발행됨** (소유자 결정 = ① 플랫폼별
  ...
  재고 close 한다.
- `TASK-2026-09-01-main-002` — **`EXPECTED_LAST_UPDATED` 하드코딩 2곳**. `check_code_index_v0_15_17` ·
  `check_document_index_v0_15_16` 이 인덱스 문서의 '최종 수정일' 을 **리터럴**로
  들고 있어, 발행 post-step(`doc-headers-update`)이 스탬프를 올릴 때마다 손으로
  맞춰야 한다 (v1.7.0 `4d7a78da` · v1.8.0 71차 — **같은 자리 두 번**). 문서에서
  파생하면 닫힌다 (71차 main-005 follow-up 을 72차가 채번).
