...
- `TASK-2026-08-24-main-003` — bootstrap 이 **v0.14.0 이전 daily 템플릿**을 낸다.
  새 프로젝트가 첫날부터 두 표기를 같이 받고(bootstrap 은 한국어, 도구는 영어),
  한 파일에 임베드 task 와 append-only 인덱스가 겹쳐 쌓인다. **표기 혼재는 증상이고
  레이아웃 불일치가 원인이다.** 재료는 `TASK-2026-08-24-main-002` 에 있다.
...
- **혼합 표기 코퍼스 일괄 마이그레이션 여부** (재료 = `TASK-2026-08-24-main-002`).
  실측으로 질문이 다시 섰다. **비용은 싸고 안전하다**: 190파일 · 2378줄 · 매핑 17개,
  앵커 치환 시 잔여 0 · 코드펜스 오탐 0 이고 **파서 출력이 before == after 로 완전히
  동일**하다(격리 사본 대조). 되돌리기는 revert 한 번. frontmatter status 는 298개 전부
  있어 **본문 라벨은 사람이 읽는 면**이지 상태의 근거가 아니다. **급하지 않은 이유**:
  레거시 파일은 정지 상태이고 별칭이 이미 둘 다 읽는다. **급한 쪽은 따로 있다** —
  생성기가 옛 형식을 내는 자리(`main-003`)가 매 bootstrap 마다 새 혼재를 만든다.
  이 결정은 그것을 고친 뒤에 해도 늦지 않다.
