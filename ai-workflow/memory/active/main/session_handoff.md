...
- 현재 주 작업 축: **문서 개편 — 영어화(①)는 끝났고 남은 둘은 구조 작업이다.** ②[TASK-2026-08-14-main-007] **handoff 기준선 롤오프** — §1 기준선이 handoff 의 66%(27.5K자)이고 세션마다 단조 증가한다. 삭제가 아니라 **이관**이어야 하고 `state.json` 생성기 입력 형식이 깨지면 안 된다. ③[TASK-2026-08-14-main-008] **task SSOT 구조화** — 여기에 **메모리 필드 라벨 영어화가 붙는다**. 라벨은 `project_docs.py` 의 `STATUS_RE` 와 writer 4곳이 emit 하는 파싱 계약이라, 형식이 어차피 바뀌는 이 작업 안에서 함께 옮기는 것이 유일하게 안전한 순서다 (257개 task + 소비자 저장소 + 2년 compat). **대기 축**: [main-004] 2축→1축 조건부(위험이 절감보다 크면 안 하는 것도 결론) / [main-005] seed 의 `sessions/` gap / [main-006] 아카이브의 살아있는-대상 링크 재작성 / 정숙 구간 `no_repo_write` 39s / [TASK-2026-08-13-main-004] mypy flake 33 run / macOS PEP 668 / cross-host federation.
