...
- 그 이전 기준선은 [`baselines.md`](./baselines.md) 에 있다 (이관 66건, 최신이 위).
...
- **혼합 표기 코퍼스 일괄 마이그레이션 여부** (재료 = `TASK-2026-08-24-main-002`).
  ...
  **그 자리는 `main-003` 이 닫았다 (2026-08-24)** — 새로 생기는 혼재가 멈췄으므로
  이제 이 결정은 급하지 않다. 남은 것은 정지한 레거시 190파일뿐이고, 별칭이
  이미 둘 다 읽는다.
