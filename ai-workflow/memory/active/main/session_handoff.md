...
- 현재 주 작업 축: **task SSOT 구조화 3단계 — 본문 라벨 영어화** ([TASK-2026-08-14-main-008], 1·2단계 완료). **선행 조건이 갖춰졌다**: `status` 는 1단계에서 frontmatter 가 정본이 됐고, 열거형 필드는 2단계에서 인자→렌더→교체가 **라벨 문자열 하나에만** 의존하게 모였다(`_set_list_field(lines, label, values)`). 3단계의 첫 일은 **라벨 리터럴을 한 자리로 모으는 것** — 아직 `backlog_update` / `read_only_bundle` / `workflow_writes` 에 흩어져 있다. 모으고 나면 라벨 교체는 정본 상수 한 곳을 옮기는 일이 된다. **대기 축**: [main-004] 2축→1축 조건부(위험이 절감보다 크면 안 하는 것도 결론) / [main-005] seed 의 `sessions/` gap / [main-006] 아카이브의 살아있는-대상 링크 재작성 / 정숙 구간 `no_repo_write` 39s / [TASK-2026-08-13-main-004] mypy flake 33 run / macOS PEP 668 / cross-host federation.
