...
- 상태: active
- 최종 수정일: 2026-09-21 (81차 세션 **종료** — '범위가 조용히 좁으면 그 밖이 갈라진다' 4건 close: goal coverage 어휘→선언 / wiki_trend 시계 의존 / ENFORCES 축 신설 / smoke 수 포함목록→전수)
...
> **`TASK-2026-09-21-main-005` (Python 문법 호환 축) 이 열려 있다.** 기대 하한은
> **CI 매트릭스가 아니라 `requires-python` 선언**에서 파생해야 한다 — CI 는
> 3.10/3.11/3.12 를 섞어 쓰므로 CI 를 기대값으로 삼으면 선언과 갈라진다.
> 착수 시점에 이미 실측해 뒀다: 소스 551개가 3.10/3.11/3.12 전부 파싱 실패 0 이므로
> **고칠 것이 아니라 막을 것**이다. 예방 축이라는 사실을 잊으면 "왜 red 가 한 번도
> 안 났나" 로 오해하게 된다.
