...
> **`TASK-2026-09-22-main-003` — `runtime_load` 의 mtime 폴백이 채널마다 다르게 틀린다 (planned).**
> antigravity 는 **병합 복사**라 최상위 디렉터리 mtime 이 안 움직인다. v1.10.0
> 재적용 직후 내부 파일은 `09-22 03:01` 인데 디렉터리는 `09-18 00:16` 그대로였고,
> 탐침이 그 값을 설치 시각으로 보고했다 — 그 사이에 뜬 호스트를 **'최신' 으로 센다.**
> codex·grok-build 는 디렉터리를 새로 만들어 갱신됐다. **판정이 채널의 설치 방식에
> 따라 조용히 틀린다.** 틀린 값보다 미측정이 낫다는 선택지도 같이 둔다.
...
> 위 둘(`main-002` · `main-003`)과 `main-001` 은 전부 v1.10.0 발행 준비·실행
> 중에 드러났다. **발행 절차를 밟는 것 자체가 탐침이었다** — 평소 경로로는 안
> 밟히는 자리들이다.
