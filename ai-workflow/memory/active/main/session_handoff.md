...
> **`TASK-2026-09-22-main-001` — `consumer-metrics-digest` 가 8월부터 만성 red (planned).**
> `REQUIRED_CI_WORKFLOWS` 가 아니라 발행을 막지 않지만 8회 중 7회 failure 다.
> 실패는 'Post to existing issue' 한 단계이고 `permissions.issues=write` 선언과
> 대상 issue #22 · 라벨은 모두 존재한다. `gh run view --log` 가 빈 출력이라
> **실제 stderr 를 아직 못 봤다** — 추측으로 고치지 않는다.
>
> 이 셋은 전부 v1.10.0 발행 준비·실행 중에 드러났다. **발행 절차를 밟는 것 자체가
> 탐침이었다** — 평소 경로로는 안 밟히는 자리들이다.
