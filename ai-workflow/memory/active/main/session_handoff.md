...
**조사가 어디까지 보는지를 선언하고 있었다 — 포함 목록을 없앤다 (TASK-2026-08-01-main-001, §2.53).**
...
- [ ] **제외는 여전히 이름 기반**(`EXCLUDED_PARTS`). `build` 라는 이름의 진짜 소스
      디렉터리가 생기면 조용히 빠진다. 성질(생성물인가)로 가르는 판정 근거가 아직 없다.
