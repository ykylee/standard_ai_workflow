...
- 그 이전 기준선은 [`baselines.md`](./baselines.md) 에 있다 (이관 64건, 최신이 위).
...
- `check_watch_transient_writer` 병렬 부하 flake — 2026-08-22 전량 native 축에서
  1회 관측(`test_transient_reinjection_is_captured`), 단독 재현 안 됨. main-005 가
  고정 sleep 을 관측 대기로 바꾼 뒤에도 남은 잔여다. **다음 재발 시 상세를 남길 것.**
