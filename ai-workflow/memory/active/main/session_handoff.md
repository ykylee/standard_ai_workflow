...
> **`TASK-2026-09-21-main-006` — 검사를 CI 인터프리터로도 돌리는 축 (planned).** main-005 가 겨냥한
> 결함('로컬 green + CI red')을 **그 축을 만들다가 그대로 재현했다** — case 3 이
> 호스트 의존인 것을 모르고 무조건으로 단언해 로컬 3.13 green / CI 3.11 red 가 났다
> (`9915e4ad`). 저장소에는 브랜치 컨텍스트 매트릭스는 있어도 검사 자신을 CI
> 인터프리터로 돌리는 축이 없다. 지금의 유일한 수단은 push 전에
> `uv python find 3.11` 로 한 번 더 돌리는 것이고 **사람의 기억에 의존한다.**
> 이것으로 '로컬 green + CI red' 는 이 저장소에서 네 번째다 (SDK 매트릭스 ·
> 브랜치 매트릭스 · Python 하한 · 그리고 이번).
>
> 착수 전 실측 두 개를 task 에 박아 뒀다: **기전** — 러너가 검사를 `sys.executable`
> 로 띄우므로(573행) 러너를 다른 인터프리터로 돌리면 축이 선다(별도 배선 불필요).
> **비용** — uv 로 3.11 venv + requirements 전체 설치가 **0.648s** 였다. 즉 비용의
> 본체는 환경 구성이 아니라 **게이트 벽시계**(전량 2축을 더 돌면 6.5분→13분)이고,
> CLAUDE.md 가 그 벽시계를 명시적으로 관리하므로 전량 반복은 기각 후보다.
> 설계 3안(전량 / 선언된 부분집합 / 하이브리드)은 실측으로 고른다.
