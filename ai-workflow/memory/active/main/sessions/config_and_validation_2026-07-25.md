...
## 📌 푸시 후 CI 결과 (2026-07-25 15:07, `6fe7934`)

| 워크플로우 | 결과 |
|---|---|
| mypy-strict | ✅ — **2개월 만에 처음으로 진짜 strict 로 통과** |
| actionlint (신규) | ✅ 12s |
| mcp-inspector (신규) | ✅ 33s — 서버 실기동 후 descriptor 대조 |
| mkdocs | ✅ 34s |
| smoke | ❌ **1건** — `check_release_summary_v0_11_15` |
| okf-validate | ❌ — **기존 결함**, 내 변경과 무관 |

- `check_mypy_ci_cross_verify` 는 예상대로 풀렸다 (같은 SHA 의 mypy-strict success).
- `check_release_summary` 는 기록돼 있던 **타이밍 경합** — 두 워크플로우가 동시에 시작해
  smoke 가 그 체크에 닿는 시점에 아직 CI 결과가 없으면 `ci_stale` 이 된다.
  푸시 뒤 로컬에서 두 체크 모두 1/1 PASS 로 전환됨을 확인했다.
- `okf-validate` 는 `--cache` 옵션이 CLI 에 없어 `ambiguous option` 으로 죽는다.
  그 플래그는 `03cbfb0`(내 변경 전)에도 있었고 **최소 2026-07-21부터 매 실행 실패**했다.
  또 하나의 "오래 red 인데 아무도 안 보는" 워크플로우다.

## ⏭️ Next Actions

- [ ] **smoke 아티팩트의 `error_excerpt` 가 실패 사유를 놓친다** — `run_all_checks.py:299`
      의 `"".join(output.split("\n")[-3:])` 는 출력이 개행으로 끝나므로 실제로는
      `['', '=== Result: 0/1 PASS ===', '']` 를 잡는다. 사유가 적힌 `✗ ... FAIL: <이유>`
      줄은 **뒤에서 4번째**라 항상 잘린다. 이번에 CI 실패 사유를 아티팩트에서 볼 수
      없어 로컬 재현으로 알아냈다 — §2.27 과 같은 부류가 한 겹 남아 있다.
      (구분자 없는 `"".join` 도 함께 볼 것: 여러 줄이 붙어 한 줄이 된다.)
- [ ] **`okf-validate` 의 `--cache`** — 존재하지 않는 옵션. `--body`/`--ttl` 등으로
      의도를 다시 표현해야 한다. 2026-07-21 이후 매 실행 red.
