...
def copy_core_docs(paths: Paths, *, force: bool) -> list[str]:
    ...
    for name in DEFAULT_CORE_DOCS:
        source = REPO_ROOT / "ai-workflow"/ "core"/ name
    ...
    for raw_relative_path in DEFAULT_CORE_SUPPORT_PATHS:
        ...
        source = REPO_ROOT / "ai-workflow"/ relative_path
        destination = paths.kit_root / relative_path
        if source.is_dir():
            for file_path in sorted(source.rglob("*")):
                if not file_path.is_file():
                    continue
                nested_relative = file_path.relative_to(REPO_ROOT / "ai-workflow")
...
def global_snippet_sources() -> dict[str, dict[str, str]]:
    return {
        "codex": {
            "readme": str((REPO_ROOT / "ai-workflow"/ "global-snippets"/ "codex"/ "README.md").resolve()),
            "snippet": str((REPO_ROOT / "ai-workflow"/ "global-snippets"/ "codex"/ "config.toml.snippet").resolve()),
            "target": "~/.codex/config.toml",
            "policy": "additive_only",
        },
        "opencode": {
            "readme": str((REPO_ROOT / "ai-workflow"/ "global-snippets"/ "opencode"/ "README.md").resolve()),
            "snippet": str((REPO_ROOT / "ai-workflow"/ "global-snippets"/ "opencode"/ "opencode.global.jsonc").resolve()),
...
- [ai-workflow/memory/state.json](./ai-workflow/memory/state.json)
- [ai-workflow/memory/session_handoff.md](./ai-workflow/memory/session_handoff.md)
- [ai-workflow/memory/work_backlog.md](./ai-workflow/memory/work_backlog.md)
- [ai-workflow/memory/backlog/{args.today}.md](./ai-workflow/memory/backlog/{args.today}.md)
{generated_assessment}
...
def load_template(name: str) -> str:
    template_path = REPO_ROOT / "ai-workflow"/ "templates"/ name
