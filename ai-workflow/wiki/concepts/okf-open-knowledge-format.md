...
- [[concepts/wiki-source-rule-r9]] — R9 rule (`last_ingested_from`) 가 OKF 의 `resource` 와 의미 중복, export 시 매핑 필요
- [[concepts/stage-gate-pattern]] — output stage 의 structured payload 와 OKF 의 frontmatter 권장 field 부분 정합
- [[concepts/contract-v1-output-validation]] — orchestrator-subagent contract 와 OKF 의 concept-level metadata 정합 검토
- [[decisions/adr-001-3-layer-separation]] — Source/Runtime/Project 3-layer 분리 정책과 OKF 의 source-vs-bundle 분리 비교
- [[patterns/r4-anchor-index]] — wiki anchor 기반 index 와 OKF reserved `index.md` 의 progressive disclosure 패턴 유사
- [[patterns/wiki-stub-emit]] — wiki stub emit 패턴이 OKF 의 "minimal example bundle" (§Appendix A) 과 직접 매핑 가능
- [[topics/wiki-ingest-lifecycle]] — ingest cycle 과 OKF 의 knowledge graph emergent model 정합 검토
