...
| 2026-06-16 | 0.2.1 | **v0.7.47 release: 추가 evidence.** `v0.7.45 follow-up bundle` (per-strategy cache_stats_per_strategy_with_hit_rate + split_to_per_strategy) + `v0.7.46 follow-up bundle` (cache_size_compare + CLI --per-strategy test fix) + `v0.7.47 follow-up bundle` (cache_lfu_decay + cache_analytics, this turn). 1 release cycle 의 운영 evidence 후 본 release (v0.7.47) 시점에 revision log v0.2.1 entry. ADR-024 *follow-up* 의 *status*: stable. | Sisyphus (orchestrator) |
