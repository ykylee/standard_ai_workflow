...
| `code-worker` | 범위가 명확한 코드/설정 수정 | 지정 파일, 변경 요구, 검증 기준 | bounded patch, 핵심 리스크 |
...
- 네트워크 fetch
- 운영 원칙:
- 간단한 triage 와 경로 확인은 직접 수행할 수 있다.
- 대량 파일 읽기, 비교, 초안 생성, 실제 수정은 서브 에이전트로 넘기는 편을 기본값으로 둔다.
...
- `code-worker`: 코드 수정, 설정 수정, 범위가 좁은 리팩터링, 테스트 보강에 우선 사용한다.
...
- `code-worker`:
- 기본 권장 모델: `small`
- 예외: 아키텍처 변경, 범위가 넓은 리팩터링, 높은 회귀 위험 수정은 `main`
