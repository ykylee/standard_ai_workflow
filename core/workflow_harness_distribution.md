...
OpenCode 타겟은 `opencode.json` 과 project-local `.opencode/` 디렉터리 구성을 사용한다.
...
- `opencode.json` 의 `instructions` 는 공통 workflow 문서를 직접 가리킨다.
...
- worker 는 generic 하나만 두기보다 문서, 코드, 검증 성격으로 나눌수록 컨텍스트 오염과 권한 범위를 더 잘 제어할 수 있다.
