...
└── ai-workflow/            # 워크플로우 메타 레이어 (상태/백로그 관리)
```

## 2. 핵심 컴포넌트

### 🛠️ Workflow Kit (`workflow_kit/`)
...
- **부트스트랩**: `scripts/bootstrap_workflow_kit.py`
- **상태 생성**: `scripts/generate_workflow_state.py`
- **테스트 실행**: `tests/check_docs.py`, `tests/check_bootstrap.py`

## 4. 에이전트 활용 팁
- 코드 수정 시 `workflow_kit/common/`의 유틸리티를 먼저 확인하여 중복 구현을 방지하십시오.
- 새로운 스킬 추가 시 `skills/` 하위의 기존 패키지 구조를 준수하십시오.
