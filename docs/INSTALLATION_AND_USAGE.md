...
- 상태: stable (v1.3.0-beta 기준; 일부 본문 예시는 v0.5.10 시점 baseline 으로 표기, 동작 자체는 v1.1.6 과 정합)
...
- 264개 스모크 테스트 (`workflow-source/tests/check_*.py`) 실행 방법 (v1.1.6+ 정합)
...
저장소에는 264개의 `workflow-source/tests/check_*.py` 가 있다. CI는 매 push 마다 이 전부를 돌린다.
...
노출 여부는 세션이 켜져 봐야 알 수 있어 탐침 밖이다. 그래서 `content_drift` 는
그것을 `declared_unmeasured` 로 **밝힌다** — 재지 못하는 것을 통과로 세지 않는
원칙은 §7.0.0 의 `installable` 과 같다. 스킬이 안 보이면 파일 비교로 끝내지 말고
**새 세션에서 실제 호출**로 확인한다.
