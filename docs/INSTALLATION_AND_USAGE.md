...
- 상태: stable (v1.1.3-beta 기준; 일부 본문 예시는 v0.5.10 시점 baseline 으로 표기, 동작 자체는 v1.1.3 과 정합)
- 최종 수정일: 2026-07-21
...
- 259개 스모크 테스트 (`workflow-source/tests/check_*.py`) 실행 방법 (v1.1.3+ 정합)
...
저장소에는 259개의 `workflow-source/tests/check_*.py` 가 있다. CI는 매 push 마다 이 전부를 돌린다.
