...
- 상태: stable (v1.9.4 기준; 일부 본문 예시는 v0.5.10 시점 baseline 으로 표기, 동작 자체는 v1.1.6 과 정합)
- 최종 수정일: 2026-09-21 (스모크 파일 수 286 반영)
...
- 286개 스모크 테스트 (`workflow-source/tests/check_*.py`) 실행 방법 (v1.1.6+ 정합)
...
>
> ```bash
> uv tool install https://github.com/ykylee/standard_ai_workflow/releases/download/v1.9.4/standard_ai_workflow-1.9.4-py3-none-any.whl
> # 또는: pipx install <같은 URL>
> # 또는 tag 에서 직접: uv tool install "git+https://github.com/ykylee/standard_ai_workflow@v1.9.4#subdirectory=workflow-source"
> wk --help
> ```
...
저장소에는 280개의 `workflow-source/tests/check_*.py` 가 있다. CI는 매 push 마다 이 전부를 돌린다.
...
pi install ./plugin
# 또는 태그 고정 설치:
pi install git:github.com/ykylee/standard_ai_workflow@v1.9.4
```
...
> 내용 일치를 말한다. **버전 문자열이 같다는 것은 사본이 최신이라는 증거가 아니다** —
> 2026-08-31 실측: 전역 `wk`(릴리스 v1.7.0 휠)가 저장소 소스(같은 1.7.0 자칭, 미발행
> 커밋 이후)와 갈라진 채 돌면서 **지원 종료된 채널을 '막힘' 으로 · 신설 채널을 부재로**
> 보고했고, 출력엔 `kit version : 1.7.0` 한 줄뿐이었다. 사본의 버전은 **사본 옆**의
> 선언에서 읽는다 (인터프리터의 메타데이터를 읽으면 무관한 배포본의 값이 사본 버전으로
> 둔갑한다). 자산 파일은 채널마다 담기는 것이 달라 대조 단위에서 뺀다.
...
**그 한 칸을 2026-09-18 에 처음 쟀다** (TASK-2026-09-18-main-005). 재시작한
호스트에서 스킬 4종이 전부 노출되고 실제 호출이 성공했다 — 2026-08-20 의
`Unknown skill` 과 정반대 방향의 확인이다. 저장소 cwd 뿐 아니라 **중립 cwd**
에서 띄운 새 프로세스에서도 4종이 보였으므로, 노출하는 것은 프로젝트 오버레이가
아니라 user 스코프 설치다.

> **그런데 그 호출이 읽은 것은 캐시 사본이 아니었다.** 실제 호출이 보고한 skill
> base directory 가 `~/.claude/plugins/cache/<mp>/<plugin>/<version>/` 이 아니라
> **저장소 워킹 트리**(`<repo>/plugin/skills/…`)였다. 원인은 마켓플레이스 **소스
> 유형**이다 — `~/.claude/plugins/known_marketplaces.json` 에서 이 마켓플레이스는
> `source: directory`, `installLocation: <repo>` 다. 캐시 사본은 별도 inode 의
> 실사본으로 설치 시각에 만들어져 있지만 **서빙되지 않는다**.
>
> 두 사본이 지금 byte 동일이라 파일 비교로는 원리적으로 못 가른다. 그래서
> **되주입으로 확정했다**: 캐시 사본에 마커를 넣고 중립 cwd 에서 새 프로세스를
> 띄워 스킬 지시문에 그 문자열이 있는지 물었다 — 없었다 (원복 후 byte 동일 확인).
>
> 따라서 `content_drift` 의 캐시 in-sync 는 **directory 소스에서 노출의 증거가
> 아니다** — 재는 대상과 읽히는 대상이 다르다.
>
> **이제 탐침이 그것을 읽는다** (TASK-2026-09-18-main-006). `content_drift` 가
> `known_marketplaces.json` 의 소스 유형을 읽어 세 가지를 한다: ① `directory`
> 소스면 **서빙 경로를 실제로 대조**하고(캐시 대신 그쪽이 `in_sync` 판정의
> 대상이다), ② 캐시 사본을 `served: false` 로 적어 `unserved` 에 남기며 —
> 읽히지 않는 사본의 드리프트는 발견으로 세지 않는다(`superseded` 와 같은 규율:
> 아무도 안 읽는 사본으로 거짓 발견을 만들지 않는다), ③ 그 상태를 발견으로
> 말한다(오류가 아니라 상태이지만, 모르면 캐시 in-sync 를 노출의 증거로 읽게
> 된다). 플러그인이 소스 안 어디에 있는지는 `<소스>/.claude-plugin/marketplace.json`
> 의 `plugins[].source` 에서 읽는다 — `plugin/` 을 관례로 가정하지 않는다.
>
> 이 판정은 소스 유형에 달렸다: `github` 소스로 설치한 소비자 쪽은 이 호스트에서
> 잴 수 없으므로 §7.0.2 표의 claude-code 행을 통째로 뒤집어 읽지 않는다. 탐침도
> 그 셀을 **가정으로** 둔다 — 원격 소스는 캐시가 읽히는 것으로 보되, 그것을
> 되주입으로 확인한 적이 없다는 사실을 `declared_unmeasured` 에 적는다.
...
8. **이 표에 없는 축이 하나 있다 — 마켓플레이스의 *소스 유형*** (2026-09-18 실측,
   TASK-2026-09-18-main-005). claude-code 행의 "설치본의 정체 = 캐시 사본" 은
   **`github` 소스를 전제로 읽힌 것**이고, `directory` 소스(로컬 경로를
   `claude plugin marketplace add <경로>` 로 등록한 개발 호스트)에서는 성립하지
   않는다. 그 경우 하네스가 실제로 여는 것은 **소스 디렉터리 자신**이고 캐시
   사본은 만들어지되 서빙되지 않는다 — 되주입으로 확정했다(§7.0.1). 즉 그 호스트의
   claude-code 는 이 표의 claude-code 행이 아니라 **pi-dev 행("경로 참조 — 원본이
   곧 설치본")에 가깝다**: 소스를 고치면 재설치 없이 다음 **프로세스 시작**에
   반영되고, 복구 열의 `uninstall` → `install` 은 읽히지도 않는 사본을 고치는
   절차가 된다. 어느 쪽인지는 추측하지 말고 `~/.claude/plugins/known_marketplaces.json`
   의 `source` 를 읽는다. **`github` 소스 셀은 이 호스트에서 잴 수 없어 비어 있다** —
   추정으로 채우지 않는다.
