...
- 다중 워크스페이스 오케스트레이션 (v0.15.20+):
  ...
  - `python workflow-source/tests/check_registry_mavis_sync.py` (registry ↔ mavis 양방향 동기)
  - `python workflow-source/tests/check_registry_entry_env.py` (RegistryEntry.env + sync_mavis env 합성)
  - `python workflow-source/tests/check_registry_endpoint_synth.py` (endpoint → mavis alias command/url 합성)
