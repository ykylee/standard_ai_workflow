...
- 상태: stable (v1.10.0 기준; 절차 자체는 v0.5.7+ 부터 정식 도입된 정책 유지)
- 현재 package version: 1.10.0 (`workflow-source/pyproject.toml`)
- 최종 수정일: 2026-09-22
...
| v1.9.1 | `session-start` 의 상태 불일치 경고가 handoff 를 하루치 backlog 가 아니라 **task corpus 전수**와 대조한다 (분모·ID 키·차집합 설명 3겹 교정) | ✅ (2026-09-03) | §1.5 판정 **patch** — 공개 API **추가** 3건(`collect_task_corpus_status` · `diff_state_lists` · `STATE_CONFLICT_MARKER`), 제거·시그니처 변경 0 · 산출물 형식 변경 0 (`warnings` 는 그대로 `list[str]`). 경고 **문안**이 바뀐다 — 리터럴로 매칭하던 소비자는 `STATE_CONFLICT_MARKER` 를 쓴다 |
...
- [현재 릴리스 노트 v1.10.0](https://github.com/ykylee/standard_ai_workflow/blob/main/workflow-source/releases/Beta-v1.10.0.md)
