...
- bootstrap 은 `--harness` 로 하네스 선택 (10종: `codex` / `opencode` / `antigravity` / `minimax-code` / `claude-code` / `aider` / `goose` / `grok-build` / `pi-dev` / `codewhale` v0.10.4 신규), `--enable-mcp` 로 MCP 심기, `--adoption-mode {new,existing}` 로 빈/기존 프로젝트 모드 분기.
