---
type: pattern
title: R4 Anchor-Based Index Pattern
description: A knowledge index that multiple agents (or sessions) write to needs a merge-safe structure. Free-form prose in an index file causes permanent merge conflicts when two agents add entries.
tags: [status:active, wiki-type:pattern]
status: active
...
```markdown
### [page](../path/to/page.md) {#unique-anchor-id}
```

Each entry is a single line with `[link](../link.md)` and `{#anchor-id}`. Merge surface is limited to the anchor level. New entries are append-only (zero conflicts).
...
- V-4 validator (`check_wiki_index_structure.py`) — enforces `### [path](../path.md) {#id}` format
- T2 work_backlog anchor migration (v0.6.2)

## See Also

- [decisions/adr-004-wiki-layer](../decisions/adr-004-wiki-layer)
