...
> **소비자 배포처에서는 slug 자체가 틀릴 수 있었다** (2026-09-07 실측,
> `TASK-2026-09-07-main-007`). 채번이 `get_current_branch()` 를 썼는데 그 함수는
> **모듈 자신의 저장소**(`Path(__file__).parents[3]`)에 앵커를 박는다. 소스
> 배치에서는 그것이 저장소 루트라 맞는 답이 나오지만, 설치본(uv tool / 플러그인
> 캐시)에서는 `…/lib/python3.13` 이라 git 저장소가 아니고 **조용히 `"main"`** 으로
> 떨어졌다 — 오류가 아니라 그럴듯한 오답이다. 그 결과 `feature/xyz` 소비자에서
> ID 는 `TASK-…-main-001` 인데 파일은 `active/feature/xyz/` 에 쓰였고, 브랜치마다
> 같은 번호를 매겨 **이 절이 말하는 유일성이 통째로 무효**였다. 이제 채번은
> `resolve_branch_for_workspace()` 로 **이 workspace 의 git** 을 보고, 그러지
> 못했으면 그 사실을 경고로 말한다.
>
