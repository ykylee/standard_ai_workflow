...
| **`--force` server-side 이중화 (3-layer defense)** | ✅ **구현** (v0.15.27+) — `tools/install_pre_push_hook.py` (install/uninstall/status) + `workflow_kit/assets/hooks/pre-push-no-force.sh` (POSIX sh). 1st (claim_workspace.py 가 --force option 미제공) + 2nd (pre-push hook 이 사람 / 스크립트의 직접 `git push --force` 차단) + 3rd (server branch protection, 가이드). 7 case smoke ALL PASS. (TASK-2026-08-08-main-019) |
...
> 실측 기록** 이다. 규칙을 고칠 때는 표준을 고치고 스냅샷·배포본·진입점을 다시 생성한다.
