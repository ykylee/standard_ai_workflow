# Beta v0.7.20 — release coordination observability (2026-06-15)
...
## 핵심 추가 (1 follow-up 본, 0 collateral, 0 deferred) + v0.7.19 auto-bump chain 히스토리
...
**`cmd_release` 의 pre-check 통합 (v0.7.20+)**:
```python
...
**`--auto-bump` argparse flag (v0.7.20+)**:
...
**Dry-run 시 tag_pre_check_warning** (v0.7.20+):
...
v0.7.16 의 5-step re-version + cherry-pick + merge (memory #22 §Part C) 의 root cause = `cmd_release` 의 `gh release create` 호출 *직전* 의 *pre-check 부재*. v0.7.20 의 `_check_remote_tag` 가 이 부재 해소.

**3-step race 보험 (v0.7.20+)**:
...
**v0.7.16 race 의 5-step re-version 작업 (4-5분) vs v0.7.20+ 의 1-step auto-bump (<5초)** — *시간 비용 100x 단축*.
...
| Hash | Subject |
|---|---|
| `07bf145` | feat(v0.7.20): release coordination observability (_check_remote_tag + next_available_version + --auto-bump) + 7 smoke |
...
- [workflow-source/tools/release_pipeline.py](../tools/release_pipeline.py) (~1100 line, v0.7.20 본 release, --auto-bump + 3 helper)
