...
- [x] `workflow_kit.__all__` 25 entries (24 top-level public modules + `__version__`)
...
`workflow_kit.__all__` 명시 — **25 entries** (24 top-level public modules + `__version__`, verified by import).
...
- [x] `python -c "import workflow_kit; assert workflow_kit.__version__ == 'v0.8.0-beta'"` (정합)
- [x] `workflow_kit.__all__` 25 entries (24 top-level public modules + `__version__`, verified by import + smoke test)
...
- **bug 6**: `__all__` 의 fake module 16개 (예: `cache_analytics_diff` 는 *module* 이지만
  `v_r13_implementation` 은 module 아님). fix: file system ground truth — 24 *actual top-level
  modules* + `__version__` = **25 entries** 로 narrow.
...
| M | `workflow_kit/__init__.py` | 1 line literal → 99 line (SSOT, `__all__` 25 entries, 24 module re-export, version parse fallback chain) |
