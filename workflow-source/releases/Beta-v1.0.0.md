...
### 2.56. 슬래시 브랜치를 밟는 실행이 **우연**이었다 — smoke 를 2셀로

§2.55 가 남긴 후속. 결함 3건을 고쳤지만, 그 검증의 근거는 **로컬에서 env 를 손으로
덮어 돌린 것**이었다. smoke 는 `branches: ["**"]` 로 모든 브랜치에서 돌지만 개발이
거의 main 에서 이뤄지므로, **슬래시 브랜치를 밟는 실행은 아무도 보장하지 않는 우연**이다.
같은 부류의 다음 결함은 다시 오래 살아남는다.

**조치.** smoke job 을 브랜치 컨텍스트 2셀 matrix 로 만들었다.

| 셀 | 오버라이드 | 무엇을 재나 |
|---|---|---|
| `native` | 없음 | 러너의 실제 브랜치 (기존 동작) |
| `slash` | `CODEX_WORKFLOW_BRANCH=feature/ci-slash-probe` | 중첩 디렉터리 경로 |

**"branch-sensitive 한 검사 목록" 을 선언해서 그것만 돌리지 않는다.** 그 목록은 반드시
드리프트하고 빠진 것을 셀 방법이 없다 — §2.53 에서 없앤 포함 목록과 **똑같은 모양**이다.
실제로 §2.55 에서 handoff 가 이름으로 지목한 2건 중 하나는 틀렸고 세 번째는 목록에
없었다. 전량을 두 컨텍스트로 돌린다. 두 셀은 병렬이라 wall-clock 은 그대로다.

**오버라이드 키는 `CODEX_WORKFLOW_BRANCH`**(`BRANCH_ENV_KEYS` 최우선)다.
`GITHUB_REF_NAME` 을 덮으면 러너의 실제 컨텍스트까지 바뀌므로 쓰지 않는다.

**빈 값은 export 하지 않는다.** 빈 문자열을 넣으면 `_usable_branch_name` 이 None 을 내고
다음 env 키로 흘러가, "native 를 쟀다" 와 "빈 값이라 흘렀다" 가 **같은 모양**이 된다.

> **오버라이드가 먹었는지를 workflow 자신이 강제한다.** 안 먹으면 두 셀이 같은 것을
> 재면서 "2셀 green" 이라고 보고한다 — §2.50 의 `GITHUB_REF_NAME` 사고와 같은 모양으로,
> 검사가 깨지지 않고 **무력화**되는 자리다. 해석된 브랜치를 step summary 에 먼저 적고,
> 선언과 다르면 `::error::` 로 죽인다.

**검사 1종 추가**(`check_branch_scoped_memory` 9 → **10 case**, 신규 파일 없음):
`case_10_ci_runs_a_slash_branch_context`. **`case_9` 는 *코드* 가 슬래시를 감당하는지
보고, `case_10` 은 *그 검증이 CI 에서 실제로 도는지* 를 본다 — 다른 층이다.**
matrix 셀은 지우기 쉽고, 선언만 있고 검사가 없으면 드리프트한다.

되주입 2종: 슬래시 셀 제거 → `슬래시가 든 브랜치 셀이 없다`; 오버라이드 자기 검증 제거
→ `안 먹으면 두 셀이 같은 것을 재면서 '2셀 green' 이 된다`. artifact 이름도 셀마다
갈랐다(`upload-artifact@v4` 는 같은 이름의 두 번째 업로드에서 실패해, 통과한 셀의 증거가
사라진다).
