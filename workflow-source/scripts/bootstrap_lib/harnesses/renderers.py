"""deprecated shim — 구현은 :mod:`workflow_kit.bootstrap_lib.harnesses.renderers` 로 이동 (v1.1.8).
...
_SOURCE_ROOT = _Path(__file__).resolve().parents[3]
if str(_SOURCE_ROOT) not in _sys.path:
    _sys.path.insert(0, str(_SOURCE_ROOT))

import workflow_kit.bootstrap_lib.harnesses.renderers as _impl
