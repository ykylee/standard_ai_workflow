...
def _restore_state() -> None:
    """위 wiki / memory 변경 후 복원. 본 smoke 는 git checkout 으로 clean state 복원."""
    subprocess.run(
        ["git", "checkout", "HEAD", "--", "ai-workflow/wiki"],
        cwd=str(REPO_ROOT), capture_output=True, text=True, check=False,
    )
    subprocess.run(
        ["git", "checkout", "HEAD", "--", "ai-workflow/memory/active/memory_index/entries"],
        cwd=str(REPO_ROOT), capture_output=True, text=True, check=False,
    )
