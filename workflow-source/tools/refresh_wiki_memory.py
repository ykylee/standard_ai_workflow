...
def update_state_json(by_release: dict, dry: bool = True) -> list[str]:
    ...
    new_lines: list[str] = []
    rel_order = ["(v0.7.7)", "(v0.7.6)", "(v0.7.5)", "(v0.7.4)", "(v0.7.3)", "(v0.7.2)", "(v0.7.1)", "(v0.7.0)",
...
def update_work_backlog(by_release: dict, dry: bool = True) -> list[str]:
    ...
    text = p.read_text()
    new_block: list[str] = []
    for rel in ["(v0.7.7)", "(v0.7.6)", "(v0.7.5)", "(v0.7.4)", "(v0.7.3)", "(v0.7.2)", "(v0.7.1)", "(v0.7.0)"]:
        if rel not in by_release:
            continue
        ver = rel[1:-1]
        commits = by_release[rel]
...
def update_memory_log(dry: bool = True) -> str:
    ...
    text = p.read_text()
    entry = (
        "## [2026-06-14] sync | wiki raw mirror backfill (v0.6.4~v0.7.7)\n"
        "- 8 release (v0.6.4~v0.7.7) 의 state.json / work_backlog.md / wiki/log.md 갭 보강\n"
