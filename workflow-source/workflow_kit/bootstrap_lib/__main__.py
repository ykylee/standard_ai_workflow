...
from workflow_kit.bootstrap_lib.discovery import (  # noqa: E402
    COMMON_NODE_DEPS,
    COMMON_PYTHON_DEPS,
    HARNESS_NODE_DEPS,
    HARNESS_PYTHON_DEPS,
...
from workflow_kit.bootstrap_lib.harnesses.renderers import (  # noqa: E402
...
from workflow_kit.bootstrap_lib.wiki import (  # noqa: E402
    write_wiki_files,
)
from workflow_kit.bootstrap_lib.paths import (  # noqa: E402
    HarnessDefinition,
    Paths,
...
def prompt_for_harnesses(
    ...
    *,
    ...
    """
    from workflow_kit.bootstrap_lib.harnesses import HARNESS_SPECS  # local import: keep top-level lean
...
def write_harness_files(
    ...
    if "pi-dev" in harnesses:
        ...
        if getattr(args, "entry_mode", "aggressive") != "skill-only":
            from workflow_kit.bootstrap_lib.harnesses.renderers import (
    ...
    if "grok-build" in harnesses:
        ...
        if getattr(args, "entry_mode", "aggressive") != "skill-only":
            from workflow_kit.bootstrap_lib.harnesses.renderers import render_grok_build_agents
    ...
    if "aider" in harnesses:
        ...
        if getattr(args, "entry_mode", "aggressive") != "skill-only":
            from workflow_kit.bootstrap_lib.harnesses.renderers import render_aider_conventions
    ...
    if getattr(args, "enable_mcp", False):
        ...
        if "mavis" in harnesses:
            from workflow_kit.bootstrap_lib.mcp import write_mavis_global_mcp_files
