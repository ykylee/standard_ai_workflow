...
class GraphInsightsOutput(BaseModel):
    ...
    - coverage_pct + health_score + tier 정량화 — **선언 사슬**로 닿은 비율이다
      (스펙 §7.4). 해석은 `purpose_graph.compute_health_score` docstring 이 정본.
    - coverage_mode: `declared` 일 때만 수치가 뜻을 가진다. `undeclared` / tier
      `unmeasured` 는 '나쁨' 이 아니라 '못 쟀음' 이다.
...
class BacklogUpdateOutput(BaseOutput):
    ...
    scope_creep_warnings: list[str] = Field(default_factory=list)
    # v0.11.22+ Phase 3d: ADR-005 memory_index retrieval hints (optional, opt-in).
