...
그래서 규약을 아는 자리를 **하나**로 만든다. 추출은 여기서만 하고, 소비자는
이 모듈을 부른다 (소비자였던 `okf-validate.yml` 은 2026-09-23 CI workflow 폐지,
main-022 로 삭제됐다 — 이제 로컬에서 `python -m workflow_kit.frontmatter_urls` 로 부른다).
