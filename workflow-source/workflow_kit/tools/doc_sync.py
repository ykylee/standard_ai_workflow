#!/usr/bin/env python3
"""doc-sync 구현 정본 (TASK-2026-08-11-main-021).

`skills/doc-sync/scripts/run_doc_sync.py` 에 있던 것을 `tools/` 로 올렸다.
skills/ 는 pip 패키지에도 bootstrap 번들에도 포함되지 않아, 소비자 프로젝트에는
실행 가능한 경로가 아예 없었다 (TASK-020 진단). `tools/` 는 패키지에 포함되므로
`wk doc-sync` 로 노출된다. 원래 경로에는 이 모듈을 부르는 wrapper 만 남는다.
"""
